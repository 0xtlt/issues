...
var isNumber = function(subject) {
  return typeof subject === "number";
};
var isString = function(subject) {
  return typeof subject === "string";
};
var isBoolean = function(subject) {
  return typeof subject === "boolean";
};
var isObject = function(subject) {
  return Object.prototype.toString.call(subject) === "[object Object]";
};
var mathAbs = function(n) {
  return Math.abs(n);
};
var mathSign = function(n) {
  return Math.sign(n);
};
var deltaAbs = function(valueB, valueA) {
  return mathAbs(valueB - valueA);
};
var factorAbs = function(valueB, valueA) {
...
var arrayLast = function(array) {
  return array[arrayLastIndex(array)];
};
var arrayLastIndex = function(array) {
  return Math.max(0, array.length - 1);
};
var arrayFromNumber = function(n, startAt = 0) {
  return Array.from(Array(n), (_, i) => startAt + i);
};
var objectKeys = function(object) {
  return Object.keys(object);
};
var objectsMergeDeep = function(objectA, objectB) {
...
var isMouseEvent = function(evt, ownerWindow) {
  return typeof ownerWindow.MouseEvent !== "undefined" && evt instanceof ownerWindow.MouseEvent;
};
var Alignment = function(align, viewSize) {
  const predefined = {
    start,
    center,
    end
...
var Axis = function(axis, direction) {
...
var Counter = function(max, start, loop) {
  const {
    constrain
  } = Limit(0, max);
  const loopEnd = max + 1;
...
var DragHandler = function(axis, direction, rootNode, ownerDocument, ownerWindow, target, dragTracker, location, animation, scrollTo, scrollBody, scrollTarget, index, eventHandler, percentOfView, dragFree, dragThreshold, skipSnaps, baseFriction, watchDrag) {
  const {
    cross: crossAxis
  } = axis;
  ...
  function init(emblaApi) {
    ...
    const node = rootNode;
    initEvents.add(node, "dragstart", (evt) => evt.preventDefault(), nonPassiveEvent).add(node, "touchmove", () => {
      return;
    }, nonPassiveEvent).add(node, "touchend", () => {
      return;
...
var DragTracker = function(axis, ownerWindow) {
...
var PercentOfView = function(viewSize) {
  function measure(n) {
...
var ResizeHandler = function(container, eventHandler, ownerWindow, slides, axis, watchResize) {
  let resizeObserver;
  let containerSize;
  let slideSizes = [];
  let destroyed = false;
...
var ScrollBody = function(location, target, baseDuration, baseFriction) {
  let hasSettled = true;
  let bodyVelocity = 0;
...
var ScrollBounds = function(limit, location, target, scrollBody, percentOfView) {
...
var ScrollContain = function(viewSize, contentSize, snapsAligned, containScroll) {
...
var ScrollLimit = function(contentSize, scrollSnaps, loop) {
...
var ScrollLooper = function(contentSize, limit, offsetLocation, vectors) {
...
var ScrollProgress = function(limit) {
  const {
    max,
    length
  } = limit;
...
var ScrollSnaps = function(axis, alignment, containerRect, slideRects, slidesToScroll) {
  const {
    startEdge,
    endEdge
  } = axis;
...
var SlideRegistry = function(viewSize, contentSize, containSnaps, scrollContainLimit, slidesToScroll, slideIndexes) {
  const {
    groupSlides
  } = slidesToScroll;
  const {
...
var ScrollTarget = function(loop, scrollSnaps, contentSize, limit, targetVector) {
  const {
    reachedAny,
    removeOffset,
    constrain
...
var ScrollTo = function(animation, indexCurrent, indexPrevious, scrollTarget, scrollBody, targetVector, eventHandler) {
...
var SlideFocus = function(root, slides, slideRegistry, scrollTo, scrollBody, eventStore) {
...
var Vector1D = function(initialValue) {
...
var Translate = function(axis, direction, container) {
...
var SlideLooper = function(axis, direction, viewSize, contentSize, slideSizes, slideSizesWithGaps, snaps, scrollSnaps, offsetLocation, slides) {
...
var SlidesHandler = function(container, eventHandler, watchSlides) {
...
var SlidesInView = function(container, slides, eventHandler, threshold) {
...
var SlideSizes = function(axis, containerRect, slideRects, slides, readEdgeGap, ownerWindow) {
  const {
    measureSize,
    startEdge,
    endEdge
...
var SlidesToScroll = function(axis, direction, viewSize, slidesToScroll, loop, containerRect, slideRects, startGap, endGap) {
  const {
    startEdge,
    endEdge
  } = axis;
...
var Engine = function(root, container, slides, ownerDocument, ownerWindow, options, eventHandler, animations) {
  const {
    align,
    axis: scrollAxis,
...
var Animations = function(ownerWindow) {
...
var OptionsHandler = function(ownerWindow) {
...
var PluginsHandler = function(optionsHandler) {
  let activePlugins = [];
...
var EmblaCarousel = function(root, userOptions, userPlugins) {
